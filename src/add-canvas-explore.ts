...
async function trailCanvas(trailName: string, onDistanceChange: (distance: number, lat?: number, lon?: number) => void) {
  ...
  try {
    ...
    const coordinates: { lat: number; lon: number; distance: number }[] = [];
            ...
            tooltip: {
              ...
              callbacks: {
                title: (context) => {
                  ...
                  let minDistanceDiff = Math.abs(distance - coordinates[0].distance);

                  coordinates.forEach(coord => {
                    const distanceDiff = Math.abs(distance - coord.distance);
                  ...
                  onDistanceChange(hoveredDistance, closestPoint.lat, closestPoint.lon);
